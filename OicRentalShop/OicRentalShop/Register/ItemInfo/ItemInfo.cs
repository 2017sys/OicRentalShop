...
namespace OicRentalShop.Register.ItemInfo
{
    public partial class ItemInfo : UserControl
    {
        ...
        private String serchItems()
        {
            ...
            //新作旧作検索
            if (cmb_State.Text.Trim() != "")
            {
                if (cmb_State.Text == "新作")
                    sql += " AND title.TITLE_RELEASE > " + DateTime.Today.AddMonths(-1).ToString("yyyy/MM/dd");
                else if (cmb_State.Text == "旧作")
                    sql += " AND title.TITLE_RELEASE < " + DateTime.Today.AddMonths(-1).ToString("yyyy/MM/dd");
            }
            //アーティスト検索
            if (txt_ArtistName.Text.Trim() != "")
            {
                sql += " AND art.ARTIST_NAME LIKE '%" + txt_ArtistName.Text.TrimStart() + "%'";
            }
            //ジャンル検索
